...
if(file_exists($abs_us_root . $us_url_root . 'usersc/templates/' . $settings->template . '/headerhotstatuspagesstatus.php')){
require_once  $abs_us_root . $us_url_root . 'usersc/templates/' . $settings->template . '/headerhotstatuspagesstatus.php';
}else{
  //assume template has been deleted
  if(!$ignoreTemplateFix){
  if(file_exists($abs_us_root . $us_url_root . 'usersc/templates/standard/headerhotstatuspagesstatus.php')){
    $db->update('settings',1,['template'=>'standard']);
    $settings->template = "standard";
    require_once $abs_us_root . $us_url_root . 'usersc/templates/standard/headerhotstatuspagesstatus.php';
    ...
    foreach ($dirs as $d) {
      ...
      if(file_exists($d.'/header.php')){
        ...
        require_once $abs_us_root . $us_url_root . 'usersc/templates/'.$template.'/headerhotstatuspagesstatus.php';
